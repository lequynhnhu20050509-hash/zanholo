...
def zalo_encode(params, key):
    try:
        ...
        raise _exception.EncodePayloadError(
            f"Unable to encode payload! Error: {e}")
...
def zalo_decode(params, key):
    try:
        ...
        raise _exception.DecodePayloadError(
            f"Unable to decode payload! Error: {e}")
    ...
    try:
        if encrypt_type == 0:
            ...
            if len(data_bytes) >= 48:
                ...
                decryptor = AES.new(
                    base64.b64decode(key), AES.MODE_GCM, nonce=iv)
