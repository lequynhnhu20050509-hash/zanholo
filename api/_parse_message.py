...
def parse_markdown(text):
    ...
    while any(temp_text.count(char) >= 2 for char in markdown.keys()):
        ...
        sorted_markup_start_positions = dict(
            sorted(
                markup_start_positions.items(),
        ...
        for char, start in sorted_markup_start_positions.items():
            ...
            temp_text = temp_text[:start] + temp_text[start +
                                                      len(char):end] + temp_text[end + len(char):]
            break
...
def parse_html(text):
    ...
    temp_text = text
    while any(temp_text.count(char) >= 1 and temp_text.count(
            "</" + char[1:]) >= 1 for char in markdown.keys()):
        ...
        for char, start in sorted_markup_start_positions.items():
            ...
            temp_text = temp_text[:start] + temp_text[start +
                                                      len(char):end] + temp_text[end + len(char) + 1:]
            break
