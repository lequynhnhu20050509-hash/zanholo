...
class Message:
    def __init__(self, text=None, style=None, mention=None, parse_mode=None):
        ...
        if parse_mode:
            ...
            if self.parse_mode == "Markdown":
                ...
                self.text, self.parse_list = Parse(
                    self.text, self.style, "Markdown")

            elif self.parse_mode == "HTML":

                self.text, self.parse_list = Parse(
                    self.text, self.style, "HTML")

            else:
                raise ValueError(
                    "Invalid Parse Mode, Only Support `Markdown` & `HTML`")
    ...
    def __repr__(self):
        return f"Message(text={
            self.text!r}, style={
            self.style!r}, mention={
            self.mention!r}, parse_mode={
                self.parse_mode!r})"
...
class MessageStyle:
    def __new__(
        ...
        self.length = length
        self.style = style
        if not isinstance(offset, type(length)) != int:
            raise ValueError(
                "Invalid Length, Offset! Length and Offset must be integers")
