...
class State(object):
    ...
    def login(
        ...
        if cls._cookies:
            ...
            try:
                url = f"https://wpa.chat.zalo.me/api/login/getLoginInfo?imei={imei}&type=30&client_version=645&computer_name=Web&ts={
                    _util.now()}"
                response = requests.get(
                    url, headers=headers, cookies=cls._cookies)
                data = response.json()
            ...
            raise _exception.LoginMethodNotSupport(
                "Login method is not supported yet")
