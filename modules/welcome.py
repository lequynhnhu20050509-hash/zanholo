...
des = {
    'version': "2.0.1",
    'credits': "Latte",
    'description': "Bật/tắt welcome cho nhóm",
...
def handle_welcome_settings_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(command_parts) != 2:
        rest_text = (
            f"Hướng dẫn sử dụng lệnh {PREFIX}welcome:\n"
            f"• {PREFIX}welcome on: Bật thông báo sự kiện cho nhóm.\n"
            f"• {PREFIX}welcome off: Tắt thông báo sự kiện cho nhóm.\n"
    ...
    if action == "on":
        if thread_id not in allowed_groups:
            ...
            rest_text = "Nhóm này đã được bật thông báo sự kiện trước đó! 🔄"
    elif action == "off":
        if thread_id in allowed_groups:
            ...
            rest_text = "Nhóm này đã được tắt thông báo sự kiện trước đó rồi! 🔄"
    else:
        rest_text = f"Sử dụng: {PREFIX}welcome on/off"
