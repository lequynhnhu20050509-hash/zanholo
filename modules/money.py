...
def draw_money_top_menu(client, thread_id, thread_type, message_object, money_data):
    ...
    if not top_users:
        client.replyMessage(Message(text="⚠️ Chưa có ai trong bảng xếp hạng cả, Bạn ơi!"), message_object, thread_id, thread_type)
        return
    users = []
...
def show_menu_image():
    ...
    header_text = "💰 MENU QUẢN LÝ TIỀN LATTE 💰"
...
def handle_money_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if action == "set" and is_admin(author_id):
        if len(text) < 3 or len(message_object.mentions) < 1:
            response_message = "❌ Bạn ơi, nhập số tiền hợp lệ và tag @user nha!"
        else:
            amount = parse_money(text[2])
            if amount is None:
                response_message = "❌ Bạn ơi, số tiền không hợp lệ! Vd: 500K, 1M, 1B, 1BB"
            else:
        ...
        if len(text) < 3:
            response_message = "❌ Bạn ơi, nhập số tiền hợp lệ nha!"
        else:
            amount = parse_money(text[2])
            if amount is None:
                response_message = "❌ Bạn ơi, số tiền không hợp lệ! Vd: 500K, 1M, 1B, 1BB"
                ...
                save_money_data(money_data)
                response_message = f"✅ Bạn tự cộng {format_money(amount)} cho bản thân!"
    
    elif action == "reset" and is_admin(author_id):
        try:
            os.remove('modules/cache/money.json')
            response_message = "✅ Bạn đã reset toàn bộ số dư hệ thống!"
        except FileNotFoundError:
            response_message = "✅ Hệ thống đã sạch tiền!"
    
    elif action == "remove" and is_admin(author_id):
        if len(text) < 3:
            response_message = "❌ Bạn ơi, nhập số tiền hoặc all, có thể tag @user!"
        else:
        ...
        if author_id in user_cooldowns:
            ...
            if time_since_last_use < cooldown_time:
                ...
                response_message = f"⏳ Bạn ơi, đợi {int(remaining_time // 60)} phút {int(remaining_time % 60)} giây nữa để nhận tiền free nha!"
        ...
        save_money_data(money_data)
        response_message = f"🎉 Bạn đã nhận được {format_money(amount)} miễn phí từ đại lý game!"
        ...
        if len(text) < 3 or len(message_object.mentions) < 1:
            ...
            if amount is None:
                ...
                response_message = "❌ Bạn ơi, không thể chuyển số tiền âm hoặc bằng 0!"
            else:
                target_id = message_object.mentions[0]['uid']
                target_name = get_user_name(client, target_id)
                if money_data.get(author_id, 0) >= amount:
                    ...
                    response_message = "❌ Bạn ơi, số dư không đủ để chuyển!"
