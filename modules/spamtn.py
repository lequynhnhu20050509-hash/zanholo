...
def stop_war(client, message_object, thread_id, thread_type):
    ...
    is_war_running = False
    client.replyMessage(Message(text="đã tắt spam."), message_object, thread_id, thread_type)
...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Bạn không có quyền dùng lệnh này. Chỉ có admin Latte mới được sử dụng lệnh này"),
    ...
    if action == "stop":
        if not is_war_running:
            client.replyMessage(
                Message(text="⚠️ spam nội dung đã dừng lại."),
                message_object, thread_id, thread_type
            )
        else:
    ...
    try:
        with open("spam/ngonwar2.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File lag.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
