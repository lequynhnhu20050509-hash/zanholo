...
def handle_change_avatar(message, message_object, thread_id, thread_type, author_id, client): 
    if author_id not in ADMIN:
        client.replyMessage(Message("❌ Chỉ admin mới có quyền đổi avatar bot!"), message_object, thread_id, thread_type,ttl=60000)
        return
    ...
    try:
        ...
        if not (hasattr(message_object, "quote") and message_object.quote and "attach" in message_object.quote):
            client.replyMessage(Message("❌ Vui lòng reply 1 ảnh để đổi avatar nhóm!"), message_object, thread_id, thread_type,ttl=60000)
            return
        ...
        if not photo_url:
            client.replyMessage(Message("❌ Không tìm thấy URL ảnh trong reply!"), message_object, thread_id, thread_type,ttl=60000)
            return
        ...
        if file_type != "image":
            client.replyMessage(Message("❌ Chỉ hỗ trợ ảnh để đổi avatar bot!"), message_object, thread_id, thread_type,ttl=60000)
            return

        # Thông báo đang xử lý
        processing_msg = Message(text="➜ ⏳ Đang tiến hành đổi avatar bot")
        ...
        if not jpg_url:
            client.replyMessage(Message("❌ Lỗi đổi avatar"), message_object, thread_id, thread_type,ttl=60000)
            return
        ...
        if res.status_code == 200:
            ...
            client.replyMessage(Message(f"❌ Không tải được ảnh: HTTP {res.status_code}"), message_object, thread_id, thread_type,ttl=60000)
            return

        # Đổi avatar bot
        client.changeAccountAvatar(filePath=local_path)
        ...
        client.replyMessage(Message(f"❌ Lỗi: {e}"), message_object, thread_id, thread_type,ttl=60000)
        
def TQD():
    return {
        "avtbot": handle_change_avatar
    }
