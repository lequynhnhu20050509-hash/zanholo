...
class ZaloAPI(object):
    ...
    def getCurrentUser(self):    
        session = self._state._session
        try:
           user_info = self._qr_get_user_info(session, self._state._headers.get("User-Agent"))
           if not user_info.get("data", {}).get("logged"):
               raise ZaloLoginError("Chưa có phiên đăng nhập hợp lệ.")         
           return user_info['data']['info']
        except requests.exceptions.RequestException as e:
           raise ZaloLoginError(f"Lỗi mạng khi lấy thông tin người dùng: {e}")
        except Exception as e:
           raise ZaloLoginError(f"Lỗi khi lấy thông tin người dùng: {e}")
    ...
    def getReceivedFriendRequests(self):
        ...
        Returns:
            dict: Một dictionary chứa danh sách các lời mời kết bạn và các thông tin liên quan.
            
        Raises:
            ZaloAPIException: Nếu request API thất bại.
        """
        ...
        encrypted_params = self._encode(params_to_encode)
        ...
        response = self._get(url, params=final_params)
        data = response.json()
        
        if data.get("error_code") != 0:
            error_code = data.get("error_code")
            error_message = data.get("error_message") or data.get("data")
            raise ZaloAPIException(f"Error #{error_code} when getting received friend requests: {error_message}")
            
        decoded_results = self._decode(data["data"])
        
        if decoded_results.get("error_code", 0) != 0:
            inner_error_code = decoded_results.get("error_code")
            inner_error_message = decoded_results.get("error_message") or "Inner API error after decoding."
            raise ZaloAPIException(f"Error #{inner_error_code} in decoded response: {inner_error_message}")
        
        return decoded_results.get("data", {})
